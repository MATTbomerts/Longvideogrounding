...
from .loss import *
from ..utils import fetch_feats_by_index, compute_tiou, AttentionModule,EventQueryClassifier,get_targets
import time
...
class SOONet(nn.Module):
    ...
    def forward_train(self, 
        ...
        # event_lengths.sort(key=lambda x: x[1])  # 按帧数排序，从小到大进行事件按帧数进行排序

        # batch_size = 100  # 假设每次处理 10 个事件
        # abstract_events = torch.zeros(unique_events.size(0), 15, 512, device=query_feats.device)

        # for start in range(0, len(event_lengths), batch_size):
        #     batch_indices = [idx for idx, _ in event_lengths[start:start + batch_size]]
        ...
        hit_ratio=0
        classifier_target=torch.zeros((query_feats.size(0),final_event_anchor_feats.size(0)),device=query_feats.device)
        ...
        loss_dict=self.loss(event_overlaps_pred,timestamps,query_mask,classficate_result,classifier_target)  #timestamps是真实绝对的时间戳
        train_et=time.time()
    ...
    def forward_test(self,
        ...
        for bid in range(num_batches): #小query_batch
            ...
            select_abstract_feature=abstract_features[top_indices]  #top_indices对每个查询事件选择不一样，因此得到的时间区间也不一样
        ...
        all_hit_timestamps=torch.cat(all_hit_timestamps,dim=0)
    ...
    def loss(self, 
             ...
             timestamps,
             query_mask,  #query_mask如果等于1 表示该query没有命中任何事件，不参与回归损失计算
             classficate_result,
             classifier_target
             ):
